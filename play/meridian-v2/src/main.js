...
// Phaser measures and rasterizes text the moment a Text object is created, so
// starting before the webfonts resolve bakes the fallback metrics into every
// panel. Wait for document.fonts — but never longer than a beat, since the
// game must also start when opened offline or from file:// where the fonts
// will never arrive.
function startGame() {
  if (window.GAME) return;
  // Kept on window so the running game is reachable from the browser console
  // (scene list, registry, scale manager) when debugging a build.
...
if (document.fonts && document.fonts.ready) {
  const timeout = setTimeout(startGame, 2500);
